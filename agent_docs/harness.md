...
- `compat list|run|matrix|diff|vet` — mod compatibility testing; `run --launch --auto-install --save-baseline` is the full autonomous pipeline (Nexus download, requires_mcm injection, fixture restore, launch, identity handshake, sentinel assertions, launch-scoped log scan, quit); `matrix` accepts `--launch --auto-install` too — without `--launch` every runtime assertion fails (no game socket); real verdicts come from `--launch` runs or the saved baselines in `docs/compat-reports/baseline/`
