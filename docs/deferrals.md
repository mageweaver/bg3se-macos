...
| `RaycastAll` | `nil` | **Rationale corrected 2026-08-20.** The registry said its trailing by-value `ls::Optional<PhysicsSceneScopedReadLock&>` was ABI-unverified, but that is the *same* trailing parameter as `RaycastAny`, whose zeroed-optional ABI is proven GO and now shipping. The only remaining unknown is the `ls::PhysicsHitAll&` out-parameter layout — a materially smaller task than a quarantine implies. | `PHYSICS_VMT_AUDIT.md` (full signature), `RAYCAST_ABI_B4A.md` | Recover `ls::PhysicsHitAll` layout, then reuse the RaycastAny call shim |
