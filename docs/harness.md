...
**Headless mode (`--headless`):** On macOS this is a temporary windowed launch plus app hide, not renderer-level headless mode. Fullscreen BG3 creates its own macOS Space, which System Events cannot reliably escape while the game is creating video and Metal windows. For headless launches, the harness snapshots `graphicSettings.lsx`, temporarily writes `ScreenWidth=1280` and `ScreenHeight=720` (the only transient mutations), starts BG3, hides the normal app window, then restores the original settings so later manual launches keep the user's resolution. The harness never writes display-mode keys (Fullscreen, FakeFullscreenEnabled, FakeFullscreen); those belong to the user's in-game choice. The `doctor` check verifies that `FakeFullscreenEnabled=0` is present in `graphicSettings.lsx` (set by BG3 when Display Mode is set to Windowed in Options > Video) and reports a warning if missing or wrong. A brief window flash can still happen during boot. JSON output includes the hide result and `graphics_restore`; failures attempt restore before returning.
