...
## 2026-07-29 (evening) — Wave 2 COMPLETE: functor crash root-caused + fixed, menu-stall adjudicated, live exit gate PASSED

**Goal 2.2 committed `a0f3ee6`** (functor hooks re-enabled on 7209685: 10 inline hooks, BeforeDealDamage/DealDamage wired). **Goal 2.4 committed `2fcd6a2`** (execution assertions + Wave-2 tier registrations).

**🎯 SIGSEGV on first live run (18:23:09, pid 25497) — hidden result-object ABI.** Crash at `GetComponent+64` inside the hooked `ExecuteStatsFunctors(Target)` body, fault address = instruction bytes. Register forensics (x0 = the byte-exact first 8 bytes of the Interrupt variant's entry) + codex sol debugger call-site disassembly converged: **all nine `esv::functor::ExecuteStatsFunctors` overloads return `esv::functor::Result` via a hidden leading x0 out-param that demangled names don't show.** Our wrappers omitted it, shifting every argument register; the original body then dereferenced Dobby trampoline bytes as an EntityWorld*. `ProcessDealDamageFunctors` (all-explicit params) was correct. **Fixed in `ebb61e8`**: `result_out` accepted and forwarded by all 9 wrappers (never dereferenced); guard test upgraded to pin the full ABI; wrapper assembly verified via otool. Full analysis: `docs/bugs/wave2-functor-crash-analysis.md` + `wave2-functor-crash-evidence.md`. Prevention rule adopted: never infer a C++ hook ABI from a demangled symbol alone — require caller-side register setup AND callee-side consumption.

**Menu-stall regression scare adjudicated — NOT a repo regression** (3 parallel codex sol debuggers, reports: `wave2-menu-stall-analysis.md`, `wave2-menu-stall-dylib-audit.md`, `wave2-modverification-analysis.md`). Findings: (1) all post-release commits are dylib/docs/tests only; every boot-time change is passive (StatusPrototype::Init merely resolved; treasure managers on-demand; functor targets can't execute at menu; init 51ms in both good and stalled runs, zero WARN/ERROR). (2) **Every "auto-load" today was actually FocuslessInput's injected Space winning the documented Noesis focus race** (~0.3s Space→COsiris::Load in successful runs); stalls are the same race lost — 18:22 loaded WITH all 10 hooks installed, breaking the correlation. (3) The **Mod Verification dialog is a live gate**, recomputed per Continue via `DCMainMenu::OnContinueGameCommand → CheckLoadSelectedSavegame → SavegameManager::GetMissingAddons → ModManagerClient::GetModSettingsMissmatch` (refreshes mods, compares UUIDs in the live set, ignores MD5/Folder) — no profile cache; profile-dirty hypothesis contradicted (profile8.lsf mtime Dec 2025; first stall predates the crash; same six-mod dialog documented 2026-05-16). Save manifest vs modsettings.lsx: **exact match on all fields** — the live ModManager simply lacked the six UUIDs at check time. Semantic-automation targets recorded for 7209685: `TryOpenModVerificationWindow 0x1026ad600`, `DCModVerification::Continue 0x102413968`, `DCSavegames::LoadSelectedSavegame 0x1024782cc`. User resolved interactively (enabled six mods in dialog → game rewrote modsettings.lsx, reordering the six after MCM — intentional, do not revert).

**Live exit gate — ✅ PASSED (pids 38719 + 67098, fixed dylib):**
- Tier 1: **109/109** (twice, both sessions).
- Tier 2: 66/74 → **68/74** after two test-honesty fixes; the 6 residual failures are pre-existing parity gaps byte-identical to the pre-Wave-2 run (entity:CreateComponent/RemoveComponent placement, Ext.Level.Raycast* arg-shape vs test, GetAllComponentNames, GetAll) — **Wave 3 scope, not regressions**.
- **Damage events fire live: BeforeDealDamage=51 / DealDamage=51** (paired) from BURNING status ticks on the host; `Osi.ApplyDamage` does NOT route through ProcessDealDamageFunctors (functor-only path — expected).
- **Crash-regression soak: `Ext.Debug.GetHookStatus` hooks_fired=7,472** in ~15 min of status-tick traffic, zero crashes across both sessions.
- Component Hp write round-trip: passed (Entity.ComponentWrite Hp roundtrip green in tier 2).

**Two test-honesty follow-ups (this commit):**
- `stats_manager.c stats_sync`: was computing `sync_result` then returning `true` unconditionally — the exact lie Goal 2.3 claimed to remove. Now propagates `sync_result`.
- `lua_ext.c Entity.ComponentWrite.FixedStringRefused`: `esv::OriginalTemplateComponent` is save-dependent; absent fixture now passes with a note (same policy as OneFrameRefused).

**New findings queued for Wave 3:** `stats_get_type` misresolves `BURNING` as type "Weapon" (log 22:10:51.894) — statuses therefore never reach the StatusData prototype-sync path via `Ext.Stats.Sync`; investigate type resolution. Plus the 6 tier-2 parity gaps above, and semantic menu automation (hook `TryOpenModVerificationWindow` + invoke load/verify controllers instead of blind coordinate clicks — see `wave2-modverification-analysis.md` §Recommended fix).
