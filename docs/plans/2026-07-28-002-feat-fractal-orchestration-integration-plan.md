...
- **Cost control is real**: per-node/per-step model+effort overrides, subtree-shared `max_cost`, per-step `--max-budget-usd` hard-enforced for the Claude backend, all caps live-retunable.
...
- **G2 Anthropic backend: PASS** — native Claude Code backend with enforced per-step budget caps; per-step model frontmatter (haiku for mechanical children).
- **G3 Serialization: PARTIAL** — no external-resource locks. **Design rule adopted: fractal nodes never build+deploy the dylib and never launch BG3.** Live-game work stays Claude-only per the origin protocol. (Misfits: single BG3 instance + Accessibility perms + one `/tmp/bg3se.sock`; shared Steam-folder deploy target races under parallel `cmake --build`.)
...
| Mod-vetting matrix fan-out (Wave 5) | **DEFER** | Vet analysis could fan out, but the core value — autonomous vetting — breaks on the single-game-instance constraint; every child would block on the one Claude-mediated live game. Revisit only if an "external executor" pattern emerges. |
| Crash bisection (Goal 1.1) | **DEFER (effectively reject)** | Sequential search over live-game launches; fractal adds durability but zero parallelism; Claude driving the harness directly is simpler and equally capable at n=14 (~4 probes). |
| Whole-campaign / live-game orchestration | **REJECT** | Confirmed structural misfit: tmux nodes have no GUI/Accessibility access; origin plan's "live validation is Claude-only" rule stands. |

## Pilot: Goal 3.4 — Fractal Ghidra size-extraction swarm (added to Wave 3)

Prereqs: Ghidra running with GhidraMCP on :8080 (BG3 ARM64 binary analyzed); `uv tool install plasma-fractal` (pin 1.0.0); `fractal init` on a dedicated branch point.
...
**Constraints stamped into NODE.md (non-negotiable):** never run `cmake --build` or any deploy script; never launch or signal BG3; never touch `src/` — output is documentation tables only. Claude (this session) verifies the merged tables (spot-check ≥20 sizes against direct bridge queries) and only then feeds them into the generated-registry pipeline in a separate, Claude-gated step.
...
- LLM-overhead-per-work-unit: 5 agent steps per iteration; acceptable for extraction (error recovery has value), unacceptable for deterministic pipelines — keep offline gates in `setup.sh`/Claude.
- Unattended `bypassPermissions` nodes: mandatory caps on every node; work product confined to `ghidra/offsets/`.

## Sources

- **Origin:** `docs/plans/2026-07-28-001-feat-parity-closure-codex-wave-campaign-plan.md` — wave structure, exit gates, Claude-only live-game rule (all unchanged).
