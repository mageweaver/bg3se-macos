...
// the Lua gate). The callback ref is resolved HERE, at drain time, not at
// enqueue time: gated Lua code can unregister the callback and reuse the
// numeric ref between enqueue and drain, so a queued ref could invoke the
// wrong function.
...
// Drain queued IMGUI events on the main thread. Called from the fake_Event
// tick block, which already holds the Lua gate — lock order is always
// gate -> queue mutex, and the queue mutex is never held across Lua entry
// (imgui_run_event runs after it is released), so the render thread can keep
// enqueueing without deadlock.
