...
static const VersionOffsets g_offset_table[] = {
     ...
     * ------------------------------------------------------------------ */
    {
        ...
        .game_functions = {
            ...
            /* Never derived on this vintage: these three were added to the
             * typed table during the 7398727 migration and no binary of this
             * build was available to resolve them against. 0 = unknown, so
             * offset_table_game_fn() returns NULL and callers skip the feature
             * rather than jumping somewhere stale. */
            [GAME_FN_MEMORY_ALLOCATE] = 0,
            [GAME_FN_MEMORY_DEALLOCATE] = 0,
            [GAME_FN_ESV_GAMESTATEMACHINE_UPDATE] = 0,
        },
    },
    ...
    /* ------------------------------------------------------------------
     * 4.1.1.7398727-gog — the GOG build of the same game version.
     *
     * Same source, different binary. Deltas from the Steam row are not uniform
     * (-0x7880/-0x78b0 in __DATA, -0x8750/-0x80e0 in __TEXT), so this row is
     * resolved per symbol, not shifted. Generated by tools/port_offsets.py;
     * see docs/PORTING-GOG.md. The key carries the store because the version
     * string does not distinguish the builds — offset_table_init() appends it.
     * ------------------------------------------------------------------ */
    {
        .version                 = "4.1.1.7398727-gog",

        /* Singleton pointer globals (per-entry symbol/got resolution) */
        .eocserver_ptr            = 0x089bf6d8,
        .eocclient_ptr            = 0x089bd740,
        .spell_proto_mgr_ptr      = 0x089ebaa0,
        .rpgstats_ptr             = 0x089f6550,
        .resource_mgr_ptr         = 0x08ac07d0,
        .level_mgr_ptr            = 0x08a6cd90,
        .global_template_mgr_ptr  = 0x08ab94e8,
        .cache_template_mgr_ptr   = 0x08a618f8,
        .level_cache_mgr_ptr      = 0x08aa4528,
        .staticdata_mstate_ptr    = 0x083f0c38,
        .gst_ptr                  = 0x08b1e438,
        .translated_string_repo_ptr = 0x08b1e7e8,
        /* Anonymous __common slots: no symbol, derived with
         * scripts/re/migrate_anonymous_globals.py, both RESOLVED_UNIQUE.
         *   global_switches: unique ADRP+STR writer role in
         *     App::CreateGlobalSwitches (0x100c5888c); 838 refs; cross-checked
         *     by loads in Save/LoadAnalyticsConfig and InitAppConfig.
         *   osiris_interface: first ADRP+LDR after the stack-guard load in
         *     OsirisQuery; written by InitStory/ShutdownStory, read by
         *     OsirisCall/ErrorMessage.
         * Their deltas from the Steam row (-0x78b0, -0x7880) match their named
         * __DATA neighbours gst and eocserver -- a third independent check. */
        .global_switches_ptr      = 0x08b1e690,
        .osiris_interface_ptr     = 0x08aaf078,
        .status_proto_mgr_ptr     = 0x089ee950,
        .passives_ptr             = 0x089e5048,
        .interrupt_proto_mgr_ptr  = 0x089e3710,
        .boost_proto_mgr_ptr      = 0x089c2348,
        /* Not emitted by port_offsets.py: offset_manifest.json has no entry for
         * these four, which is the same gap test_offset_table_row_fields_complete
         * reports upstream. Resolved by exact nm symbol against the GOG binary.
         * Leaving them 0 disables template-bank selection, and with it entity and
         * template lookups -- 17 tier-2 failures traced back here. */
        .esv_gamestate_evtmgr_ptr = 0x08a69a00,  // esv::GameStateEventManager::m_ptr
        .ecl_gamestate_evtmgr_ptr = 0x08a1c960,  // ecl::GameStateEventManager::m_ptr
        .global_template_bank_type_ptr = 0x08966ef8, // (anon)::g_GlobalTemplateBankType
        .tls_current_bank_type_ptr     = 0x0897aa90, // (anon)::tls_CurrentBankType
        .baseapp_instance_ptr     = 0x08ae99d8,

        /* Function offsets (symbol-resolved) */
        .fn_feat_getfeats         = 0x01b4eea8,
        .fn_getallfeats           = 0x011e4fdc,
        .fn_get_background        = 0x02978f04,
        .fn_get_origin            = 0x03404560,
        .fn_get_class             = 0x0260e5d4,
        .fn_get_progression       = 0x036800f8,
        .fn_get_actionresource    = 0x0117e088,
        .fn_get_template_raw      = 0x05f94654,
        .fn_cache_template        = 0x05d23f98,
        .fn_try_get_uuid_mapping  = 0x010b6518,
        .fn_storage_tryget        = 0x0637a1ec,
        .fn_spell_proto_init      = 0x01f4c638,
        .fn_status_proto_init     = 0x01fecad4,
        .fn_interrupt_proto_get   = 0x01b70460,
        .fn_passives_get          = 0x01c04910,
        .fn_aigrid_to_tile_pos    = 0x01157054,
        .fn_aigrid_get_metadata   = 0x01140978,
        .fn_aigrid_remove_path    = 0x01157538,
        .fn_aigrid_find_path      = 0x011580e0,
        .fn_aigrid_find_path_immediate = 0x0115b380,
        .component_data_shift    = 0x00000000,
        .component_data_shift_valid = false,

        /* Stable function IDs (offsets from the image base) */
        .game_functions = {
            [GAME_FN_FIXED_STRING_CREATE] = 0x064c7eb0,
            [GAME_FN_RESOURCE_GET] = 0x060db578,
            [GAME_FN_EXECUTE_STATS_FUNCTOR] = 0x05776570,
            [GAME_FN_EXECUTE_FUNCTORS_ATTACK_TARGET] = 0x0577a450,
            [GAME_FN_EXECUTE_FUNCTORS_ATTACK_POSITION] = 0x0577a7a4,
            [GAME_FN_EXECUTE_FUNCTORS_MOVE] = 0x0577c294,
            [GAME_FN_EXECUTE_FUNCTORS_TARGET] = 0x0577d450,
            [GAME_FN_EXECUTE_FUNCTORS_NEARBY_ATTACKED] = 0x05781010,
            [GAME_FN_EXECUTE_FUNCTORS_NEARBY_ATTACKING] = 0x057826e0,
            [GAME_FN_EXECUTE_FUNCTORS_EQUIP] = 0x05783560,
            [GAME_FN_EXECUTE_FUNCTORS_SOURCE] = 0x057855c8,
            [GAME_FN_EXECUTE_FUNCTORS_INTERRUPT] = 0x0578911c,
            [GAME_FN_PROCESS_DEAL_DAMAGE_FUNCTORS] = 0x05381488,
            [GAME_FN_TRANSLATED_STRING_TRY_GET] = 0x06540fe4,
            [GAME_FN_TRANSLATED_STRING_GET] = 0x065413d8,
            [GAME_FN_TRANSLATED_STRING_ADD] = 0x0653e820,
            [GAME_FN_MESSAGE_FACTORY_GET_FREE_MESSAGE] = 0x063e39a4,
            [GAME_FN_STD_STRING_CTOR] = 0x0652bdf0,
            [GAME_FN_BINK_LOAD_VIDEO] = 0x0390e2a0,
            [GAME_FN_VALUELIST_INSERT] = 0x01c39fb4,
            // Same manifest gap as the four singletons above; nm-resolved.
            [GAME_FN_MEMORY_ALLOCATE] = 0x06504124,
            [GAME_FN_MEMORY_DEALLOCATE] = 0x065043d4,
            // nm: __ZN3esv16GameStateMachine6UpdateERKN2ls8GameTimeE @ 0x104a189f4
            [GAME_FN_ESV_GAMESTATEMACHINE_UPDATE] = 0x04a189f4,
        },
    },
};
...
void offset_table_init(void) {
    ...
    // The version string does not identify the build on its own (see
    // build_identity.h). Non-Steam rows carry a "<version>-<store>" key; Steam
    // rows keep the bare version, which is also the fallback for an
    // unidentifiable store and what the older rows use.
    const char *store = version_detect_get_store();
    char key[96];
    if (strcmp(store, "steam") == 0 || strcmp(store, "unknown") == 0) {
        snprintf(key, sizeof(key), "%s", version);
    } else {
        snprintf(key, sizeof(key), "%s-%s", version, store);
    }

    for (int i = 0; i < (int)NUM_VERSIONS; i++) {
        if (strcmp(g_offset_table[i].version, key) == 0) {
            g_active = &g_offset_table[i];
            log_message("[OffsetTable] Loaded offsets for %s", key);
            return;
        }
    }

    log_message("[OffsetTable] Build %s not in table — running in degraded mode. "
...
const char *offset_table_game_version(void) {
    if (!g_active || !g_active->version) return NULL;

    // Rows are keyed "<version>" or "<version>-<store>"; trim the suffix once
    // into a static buffer, which is safe because the active row never changes
    // after offset_table_init().
    static char base[64];
    if (!base[0]) {
        snprintf(base, sizeof(base), "%s", g_active->version);
